...
class BaseReader(object):
    ...
    def _get_relative_position(self, tokens, entity):
        relative_position = []
        # print(tokens)
        p = tokens.index(entity)

        for i in range(len(tokens)):
            if i == p:
                relative_position.append(0 + 300)
            else:
                relative_position.append(i - p + 300)
    ...
    def _convert_example_to_record(self, example, max_seq_length, tokenizer):
        ...
        position_ids1 = self._get_relative_position(tokens, '111')
        position_ids2 = self._get_relative_position(tokens, '222')

        if self.is_inference:
            Record = namedtuple('Record',
                                ['token_ids', 'text_type_ids', 'position_ids', 'position_ids1', 'position_ids2'])
            record = Record(
                token_ids=token_ids,
            ...
            Record = namedtuple('Record', [
                'token_ids', 'text_type_ids', 'position_ids', 'position_ids1', 'position_ids2', 'label_id', 'qid'
            ])
...
class ClassifyReader(BaseReader):
    ...
    def _pad_batch_records(self, batch_records):
        ...
        batch_position_ids1 = [record.position_ids1 for record in batch_records]
        batch_position_ids2 = [record.position_ids2 for record in batch_records]
        ...
        padded_position_ids1 = pad_batch_data(
            batch_position_ids1, pad_idx=self.pad_id)
        padded_position_ids2 = pad_batch_data(
            batch_position_ids2, pad_idx=self.pad_id)
        padded_task_ids = np.ones_like(
            padded_token_ids, dtype="int64") * self.task_id

        return_list = [
            padded_token_ids, padded_text_type_ids, padded_position_ids, padded_position_ids1, padded_position_ids2,
